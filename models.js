// Модели для фильтра совместимости
window.compatibilityModels = {
	Другое: {
		icon: "🛴",
		models: ['[Model 3 GTR PROF]', '[Коляска]']
	},
	Велосипеды: {
		icon: "🚲",
		models: [
			'[Zsun Model F8]',
			'[GT9]',
			'[GT10]',
			'[GT17]',
			'[Cross V-8]',
			'[Cross V-8 PRO]',
			'[Monster V-12L]',
			'[Monster V-12]',
			'[Allroud PCX10]',
			'[Fedbike GTR]',
			'[Fedbike RKS]',
			'[Model 107]',
			'[Apache Y]',
			'[Apache X PRO]',
			'[Trike 31]',
			'[Trike xMax]',
			'[Trike PRO]',
		]
	},
	Скутеры: {
		icon: "🛵",
		models: [
			'[Model 3 Standard Range]',
			'[Model 3 Long Range]',
			'[Model 9 Long Range]',
			'[Model 11 Standard Range]',
			'[Model 14 mini]',
			'[Model 15 Maxi Range]',
			'[Model 16 Maxi Range]',
			'[Model 18]',
			'[Model 20]',
			'[Model 21 Performance]',
			'[Model 29 Performance]',
			'[Model 60]',
			'[Model 70]',
		]
	},
	Трициклы: {
		icon: "🛺",
		models: [
			'[Model 108]',
			'[Model 104]',
			'[Model 30]',
			'[Model 31]',
			'[Model 34]',
			'[Model 37]',
			'[Model XL 40]',
			'[Model 90]',
			'[Model 100]',
		]
	},
	Мотоблоки: {
		icon: "🚜",
		models: [
			'[1900]',
			'[1030]',
			'[900]',
			'[500]',
			'[7000]',
			'[2000 2KNS]',
			'[2000 N]',
			'[1000]',
			'[8000]',
		]
	}
};

// ===== Модификации для нового фильтра "Модификации" (пока заглушка) =====
// Живёт в этом же файле, поэтому загрузчик в расширении менять НЕ нужно —
// он и так грузит models.js. Позже просто замените категории и пункты
// на реальные данные.
window.compatibilityModifications = {
	'Категория 1': {
		icon: '⚙️',
		models: ['[Модификация A]', '[Модификация B]', '[Модификация C]'],
	},
	'Категория 2': {
		icon: '🔩',
		models: ['[Модификация D]', '[Модификация E]'],
	},
	'Категория 3': {
		icon: '🔋',
		models: ['[Модификация F]', '[Модификация G]', '[Модификация H]'],
	},
};
